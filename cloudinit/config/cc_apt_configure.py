...
def apply_apt(cfg, cloud, target):
    ...
    if util.is_true(cfg.get("enable_mirrors_files", False)):
        generate_mirrors_files(cfg, mirrors, cloud)
...
def generate_mirrors_files(cfg, mirrors, cloud):
    """generate_mirrors_files
    create one file for every mirror for apt-transport-mirror(1)"""
    aptmir = pathlib.Path("/etc/apt/mirrors")
    util.ensure_dir(str(aptmir))
    util.write_file(str(aptmir / f"{cloud.distro.name}.list"), f"{mirrors['PRIMARY']}\n", mode=0o644)
    util.write_file(str(aptmir / f"{cloud.distro.name}-security.list"), f"{mirrors['SECURITY']}\n", mode=0o644)
